...
1. ИЗВЛЕЧЕНИЕ — Просмотри ВСЕ {chunk_count} фрагментов. Информация распределена между ними!
   Выпиши ВСЕ найденные термины, понятия, формулы по теме вопроса.

2. ОБЪЯСНЕНИЕ — Для каждого найденного пункта:
   - Дай определение (цитируй книгу)
   - Объясни простыми словами, что это значит
   - Если есть формула — приведи её и расшифруй каждый элемент
   - Если есть пример в контексте — включи его

3. СТРУКТУРА — Оформи ответ:
   - Используй нумерованные списки
   - Выделяй ключевые термины **жирным**
   - Группируй связанные понятия

❌ ЗАПРЕЩЕНО:
- Придумывать то, чего нет в контексте
- Давать слишком краткие ответы — раскрывай тему полностью
- Останавливаться на 2-3 пунктах если в контексте есть больше
