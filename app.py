...
Важно: просмотри ВСЕ 8 фрагментов, информация распределена между ними!
...
QUERY_EXPANSION_PROMPT = """Расширь поисковый запрос синонимами и связанными терминами для поиска в книге по финансовой отчётности.

Запрос: {question}

Добавь:
- Синонимы на русском
- Английские эквиваленты (gross profit, net income, retained earnings, EBITDA, operating income и т.д.)
- Связанные понятия из баланса, P&L, отчёта о движении денежных средств
- Если вопрос про "прибыль" — добавь ВСЕ виды: валовая, операционная, чистая, нераспределённая, EBITDA

Отвечай ТОЛЬКО списком слов/фраз через пробел, без пояснений.
