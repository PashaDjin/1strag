...
ШАГ 1 — СКАНИРОВАНИЕ (обязательно!):
Пройди по КАЖДОМУ из {chunk_count} фрагментов и выпиши ВСЕ термины, содержащие ключевые слова вопроса.
Например, для вопроса о прибыли ищи: "прибыль", "profit", "income", "earnings", "RE", "NI", "EBITDA" и т.д.
⚠️ НЕ ПРОПУСКАЙ фрагменты! Информация часто в неожиданных местах.

ШАГ 2 — ПОЛНЫЙ СПИСОК:
Перечисли ВСЕ найденные термины (даже если их много — это хорошо!).
Включай термины из таблиц, формул, примеров — везде.

ШАГ 3 — ОБЪЯСНЕНИЕ каждого термина:
- Определение из книги (цитата)
- Формула если есть
- Пример если есть
- Объяснение простыми словами

❌ ЗАПРЕЩЕНО:
- Останавливаться на 3-4 пунктах если в контексте больше
- Пропускать термины из таблиц или формул
- Придумывать то, чего нет в контексте

✅ ХОРОШИЙ ОТВЕТ содержит 5-10+ пунктов если они есть в контексте.
...
Подробный ответ (используй ВСЮ релевантную информацию из контекста):"""
